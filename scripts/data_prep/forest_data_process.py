...
def matching_flow(sci_names):
    ...
    for nti in no_taxon.index:
        # 可能有多個isPreferredName
        names = sci_names.iloc[nti].isPreferredName
        for nn in names.split(';'):
            if not sci_names.loc[nti,'taxonID']:
                match_name(nn, sci_names.loc[nti,'sourceScientificName'],sci_names.loc[nti,'isPreferredName'],3)
...
for p in range(0,total_page,10):
    ...
    while c < p + 10 and c < total_page:
        ...
        print('page:',c)
        # time.sleep(30)
        url = f"https://ecollect.forest.gov.tw/EcologicalTBiAOpenApi/api/Data/Get?Token={env('FOREST_KEY')}&Page={c}"
        response = requests.get(url)
        if response.status_code == 200:
            result = response.json()
            total_page = result['Meta']['TotalPages']
