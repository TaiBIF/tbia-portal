...
# 取得台博館 & 水利署
# dataset_include = dataset[dataset.publisherID.isin(pub[pub.publisherName.isin(['National Taiwan Museum','Water Resources Agency,Ministry of Economic Affairs'])].publisherID.to_list())]
...
for d in dataset.taibifDatasetID.unique():
    ...
    if len(data):
        ...
        if len(df):
            ...
            df['group'] = group
            df['rightsHolder'] = '臺灣生物多樣性資訊機構 TaiBIF'
