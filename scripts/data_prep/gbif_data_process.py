...
folder = '/tbia-volumes/bucket/gbif_v25/'
extension = 'csv'
os.chdir(folder)
...
for f in files:
    ...
    for i in df.index:
        ...
        # occurrenceID 從 externalID 抓gbifID
        # 可能有兩種格式
        # 舊 gbif:3158161905
        # 新 資料來源.TBNnid.gbifID (例：gbif.52951189.3323115358)
        if len(row.externalID.split(':')) > 1:
            occurrenceID = row.externalID.split(':')[-1]
        elif len(row.externalID.split('.')) > 1 :
            occurrenceID = row.externalID.split('.')[-1]
        else:
            occurrenceID = None
        tmp = {
